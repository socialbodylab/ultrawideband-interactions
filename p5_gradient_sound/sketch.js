...
class UWB {
  ...
  three_point(x1, y1, x2, y2, r1, r2) {
    ...
    const dx = x2 - x1;
    const dy = y2 - y1;
    const p2p = Math.hypot(dx, dy);

    // Handle edge cases
    if (p2p < 0.001) return [x1, y1];
...
function draw() {
  ...
  // Handle audio based on tag distances
  const t0 = {
  ...
  for (let i = 1; i < tag.length; i++) {
    ...
    if (i <= 5) {
      switch (i) {
        case 1:
          osc0.amp(constrain(map(d, 150, 30, 0, 0.8),0.1, 0.6), 0.1);
          break;
        case 2:
          osc1.amp(constrain(map(d, 150, 30, 0, 0.8),0.1, 0.6), 0.1);
          break;
        case 3:
          osc2.amp(constrain(map(d, 150, 30, 0, 0.8),0.1, 0.6), 0.1);
          break;
        case 4:
          osc3.amp(constrain(map(d, 150, 30, 0, 0.8),0.1, 0.6), 0.1);
          break;
        case 5:
          osc4.amp(constrain(map(d, 150, 30, 0, 0.8),0.1, 0.6), 0.1);
          break;
      }
    }
  }
...
function drawPixelatedGradient() {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      // Calculate center of this square for color sampling
      const centerX = x + GRID_SIZE / 2;
      const centerY = y + GRID_SIZE / 2;

      // Calculate blended color for this square
      const [r, g, b] = calculateBlendedColor(centerX, centerY);

      // Set fill color and draw square
      fill(r, g, b);
