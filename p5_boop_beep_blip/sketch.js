...
let dataRate = 0;
let beep, boop, blip, allTogether, liquid;
let playing = {
  beep: false,
  boop: false,
  blip: false,
...
function preload() {
  beep = loadSound("assets/B1.wav");
  boop = loadSound("assets/B2.wav");
  blip = loadSound("assets/B3.wav");
  allTogether = loadSound("assets/Descending.wav");
  // liquid = loadSound("assets/watering.wav");
}
...
function draw() {
  ...
  const t1 = {
    x: tag[1].x * cm2p + x_offset,
    y: tag[1].y * cm2p + y_offset,
  };

  const t2 = {
    x: tag[2].x * cm2p + x_offset,
    y: tag[2].y * cm2p + y_offset,
  };
  
  const t3 = {
      x: tag[3].x * cm2p + x_offset,
      y: tag[3].y * cm2p + y_offset,
    };
  
  const t4 = {
      x: tag[4].x * cm2p + x_offset,
      y: tag[4].y * cm2p + y_offset,
    };

  // if (tag[1].status && tag[3].status) {
  //   if (dist(t1.x, t1.y, t3.x, t3.y) < 40) {
  //     playSound(liquid, "liquid");
  //   } else {
  //     liquid.stop();
  //   }
  // }

  if (tag[0].status && tag[1].status && tag[2].status) {
    if (
      dist(t0.x, t0.y, t2.x, t2.y) < 40 &&
      dist(t0.x, t0.y, t1.x, t1.y) < 40
    ) {
      playSound(allTogether, "allTogether");
    } else {
      if (dist(t0.x, t0.y, t1.x, t1.y) < 40) {
        playSound(boop, "boop");
      }
      if (dist(t1.x, t1.y, t2.x, t2.y) < 40) {
        playSound(blip, "blip");
      }
      if (dist(t0.x, t0.y, t2.x, t2.y) < 40) {
        playSound(beep, "beep");
      }
    }
  }
...
function drawAnchorsBounds() {
  ...
  noFill();
  strokeWeight(4);
  rect(x_offset, y_offset, A2X * cm2p, A2Y * cm2p);
...
function playSound(s, name) {
  if (s.isPlaying() || playing[name]) return;
